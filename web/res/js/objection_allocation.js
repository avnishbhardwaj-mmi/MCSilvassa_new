...
//    $("#finance_year").SumoSelect({csvDispCount: 3, search: true, searchText: 'Enter here.'});
    ...
    $("#forward_to_users").SumoSelect({csvDispCount: 3, search: true, searchText: 'Enter here.'});

    ALLOCATE_OBJ.resetOccupier();
    ALLOCATE_OBJ.resetOwner();
    ALLOCATE_OBJ.resetPropertyId();
    ALLOCATE_OBJ.resetUsers();
    ...
    $.post("loadZones", {}, function (result) {
        if (result != undefined) {
            ...
            for (var i = 0; i < result.length; i++) {
                ...
                ALLOCATE_OBJ.zoneMaster[result[i].zoneId] = result[i].zoneName;
            }
            $("#zone").html(html);
    ...
    DATEPICKER.init();
    ALLOCATE_OBJ.showPendingObjectionStatus();
});
...
//    Added By Jay End
    ...
    getObjectionHandlerUsers: function () {

        LOADER.show();

        $.ajax({
            url: "getObjectionHandlerUsers",
            type: 'post',
            data: '',
            success: function (result) {
                if (result !== null) {
                    ...
                    for (var r in result) {
                        var doc = result[r];
                        //console.log(doc);
                        html += "<option value='" + doc.userId + "'>" + doc.username + "</option>";
                    }
                    //console.log(html);
                    ALLOCATE_OBJ.resetUsers(html);
                }
                LOADER.hide();
            },
            error: function (e) {
    ...
    resetUsers: function (html) {
        if (html === undefined) {
            $("#forward_to_users").html("<option value='-1'>--Select User--</option>");
            $("#forward_to_users").closest(".SumoSelect").find(".CaptionCont").addClass("disable_cls");
        } else {
            $("#forward_to_users").html(html);
            $("#forward_to_users").closest(".SumoSelect").find(".CaptionCont").removeClass("disable_cls");
        }
        $("#forward_to_users").SumoSelect().sumo.unload();
        $("#forward_to_users").SumoSelect({csvDispCount: 3, search: true, searchText: 'Enter here.'});

        if (html === undefined) {
            $("#forward_to_users").closest(".SumoSelect").find(".CaptionCont").addClass("disable_cls");
            $("#forward_to_users").prop("disabled", true);
        } else {
            $("#forward_to_users").prop("disabled", false);
            $("#forward_to_users").closest(".SumoSelect").removeClass("disabled");
            $("#forward_to_users").closest(".SumoSelect").find(".CaptionCont").removeClass("disable_cls");
        }
        ...
        if (zone_id === "-1") {
            ...
            $.post("getObjectedPropertyForSearch", {zoneId: zone_id, ward: ward}, function (result) {
                ...
                if (result.Status === "200") {
                    ...
                    ALLOCATE_OBJ.resetPropertyId(html_prop);
                    ...
                    ALLOCATE_OBJ.resetOwner(html_own);
                    ...
                    ALLOCATE_OBJ.resetOccupier(html_occ);
    ...
    preservePropertyetails: function (detailArr) {
        ...
        if (detailArr === undefined || detailArr === null) {
            ...
            for (var dt in detailArr) {
                ...
                if (det != null) {
                    ALLOCATE_OBJ.propertyMap[det.propertyUniqueId] = det;
                }
            }
        }
    ...
    searchProperty: function (obj) {
        ...
        ALLOCATE_OBJ.searchResults = {};
        LOADER.show();
        $.post("searchObjectedProperty", {
            jsonViewCrit: JSON.stringify(obj)
        }, function (result) {


            ALLOCATE_OBJ.preservePropertyetails(result);
            ...
            if (result !== undefined && result.length > 0) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    ALLOCATE_OBJ.searchResults[result[i].propertyUniqueId] = result[i];

                    tab_html += "<tr><td title='Click here to view more detail' ><a href='javascript: void(0)' onclick=ALLOCATE_OBJ.populatePropertyDetail('"
                    ...
                    tab_html += "<td>" + (ALLOCATE_OBJ.zoneMaster[prop.zoneId] === undefined ? "N/A" : ALLOCATE_OBJ.zoneMaster[prop.zoneId]) + "</td>";
                    tab_html += "<td>" + (ALLOCATE_OBJ.wardMaster[prop.ward] === undefined ? "N/A" : ALLOCATE_OBJ.wardMaster[prop.ward]) + "</td>";
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.Status === "200") {
                    ...
                    ALLOCATE_OBJ.objectionId = objBean.objectionId;
                    ALLOCATE_OBJ.objectionBean = objBean;
                    ...
                    ALLOCATE_OBJ.populateEditField(objBean.objectionBeans);
                    ALLOCATE_OBJ.getObjectionActionHistory(propertyId, objBean.objectionId);
                }
    ...
    populatePropertyDetail: function (propertyId) {
        ...
        ALLOCATE_OBJ.clearPropertyDetail();
        ALLOCATE_OBJ.clearAll();
        ALLOCATE_OBJ.fillDocument();
        //Added By Jay
        ALLOCATE_OBJ.getVerifyingRelations();
        //Added By Jay End
        ALLOCATE_OBJ.propertyId = propertyId;
        var prop = ALLOCATE_OBJ.searchResults[propertyId];
        ...
        if (prop === undefined) {
            ...
            $("#prop_zone").html((ALLOCATE_OBJ.zoneMaster[prop.zoneId] === undefined ? "" : ALLOCATE_OBJ.zoneMaster[prop.zoneId]));
            $("#prop_ward").html((ALLOCATE_OBJ.wardMaster[prop.ward] === undefined ? "N/A" : ALLOCATE_OBJ.wardMaster[prop.ward]));
            ...
            ALLOCATE_OBJ.populateObjections(propertyId);
            ALLOCATE_OBJ.showObjectionWindow();
        }
    ...
    populateEditField: function (objArr) {
        ...
        for (var o in objArr) {
            ...
            html_ += "</tr>";
            ALLOCATE_OBJ.editedValueArray.push(obj);
        }
        $("#edit_field_table tbody").html(html_);

    },
    clearAll: function () {
        ALLOCATE_OBJ.editedFields = {};
        ALLOCATE_OBJ.editedValueArray = new Array();
        ALLOCATE_OBJ.floorDetailsObj = {};
    ...
    getFieldId: function () {
        ...
        if (ALLOCATE_OBJ.isComplexCategory()) {
    ...
    verifyAndCommiteObjection: function () {
        var obj = new Object();
        obj.propertyId = ALLOCATE_OBJ.propertyId;
        obj.effectingTAX = "";
        ...
        obj.objectionBeans = [];
        var keyArr = Object.keys(ALLOCATE_OBJ.editedFields);
        for (var k in keyArr) {
            var key = keyArr[k];
            obj.objectionBeans.push(ALLOCATE_OBJ.editedFields[key]);
        }
        ...
        if (obj.appliedBy.trim() === "") {
            ...
            ALLOCATE_OBJ.commiteObjection(obj);
        }

    },
    commiteObjection: function (decision, remarks, forwardTo) {

        LOADER.show();

        var obj = {};
        obj.propertyId = ALLOCATE_OBJ.propertyId;
        obj.objectionId = ALLOCATE_OBJ.objectionId;
        obj.decision = decision;
        obj.remarks = remarks;
        obj.forwardTo = forwardTo;
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.status === "200") {
                    ...
                    ALLOCATE_OBJ.showMessageWindow();
                    ALLOCATE_OBJ.removeCurrentOpenRow();
                } else {
                    alert(result.msg);
                }
            },
    ...
    preserveOBJECTIONDetails: function (detailArr) {
        ALLOCATE_OBJ.objectionMap = {};
        if (detailArr === undefined || detailArr === null) {
        } else {
            for (var dt in detailArr) {
                var det = detailArr[dt];
                if (det !== null) {
                    ALLOCATE_OBJ.objectionMap[det.propertyId] = det;
                }
            }
        }
    ...
    preserveCATEGORYDetails: function (detailArr) {
        ALLOCATE_OBJ.categoryMap = {};
        if (detailArr === undefined || detailArr === null) {
        } else {
            for (var dt in detailArr) {
                var det = detailArr[dt];
                if (det !== null) {
                    ALLOCATE_OBJ.categoryMap[det.category] = det;
                }
            }
        }
    ...
    printObjection: function () {
        //var obj = ALLOCATE_OBJ.objectionMap[ALLOCATE_OBJ.propertyId] ;   
        ALLOCATE_OBJ.keyArr = {};
        var viewObjCrit = new Object();
        viewObjCrit.objectionId = ALLOCATE_OBJ.objectionId;
        ALLOCATE_OBJ.mywindow = window.open('', 'PRINT', 'height=400,width=600');
        $.post("getObjectionCategory", {
            jsonViewCrit: JSON.stringify(viewObjCrit)
        }, function (result) {
            LOADER.show();

            ALLOCATE_OBJ.masterAttrArr = [];
            ALLOCATE_OBJ.preserveCATEGORYDetails(result.objCatList);
            var categoryArr = ALLOCATE_OBJ.categoryMap;
            ALLOCATE_OBJ.keyArr = Object.keys(categoryArr);
            for (var k in ALLOCATE_OBJ.keyArr) {
                var key = ALLOCATE_OBJ.keyArr[k];
                ALLOCATE_OBJ.masterAttrArr.push(key);
            }

            ALLOCATE_OBJ.printPreview(ALLOCATE_OBJ.propertyId, ALLOCATE_OBJ.masterAttrArr);
            LOADER.hide();
        }, 'json').always(function () {
        });
    ...
    printPreview: function (property_id, masterAttrArray) {
        var obj = ALLOCATE_OBJ.objectionBean;
        //console.log(obj);
        if (obj !== undefined) {
            var prop = ALLOCATE_OBJ.searchResults[property_id];

//            console.log(ALLOCATE_OBJ.editedValueArray);
            var print_html = "";
...
//            mywindow.close();
            ...
            for (i = 0; i < ALLOCATE_OBJ.editedValueArray.length; i++) {

                print_html += " <tr>";
                print_html += "<td colspan='2'><center><span style='font-weight:bold'>" + ALLOCATE_OBJ.editedValueArray[i].attrName + ":</span></center></td>";
                print_html += "</tr>";
                ...
                print_html += " <td style='border: none;  border-left: 1px solid black; text-align: center;'>" + ALLOCATE_OBJ.editedValueArray[i].value + "</td> </tr> </table></td>";
                ...
                print_html += " <td style=' border: none;  border-left: 1px solid black; text-align: center;'>" + ALLOCATE_OBJ.editedValueArray[i].newValue + "</td> </tr> </table></td></tr>";
...
//            var mywindow = window.open('', 'PRINT', 'height=400,width=600');
            ...
            var is_chrome = Boolean(ALLOCATE_OBJ.mywindow.chrome);
            ALLOCATE_OBJ.mywindow.document.write(print_html);

            if (is_chrome) {
                setTimeout(function () { // wait until all resources loaded 
                    ALLOCATE_OBJ.mywindow.document.close(); // necessary for IE >= 10
                    ALLOCATE_OBJ.mywindow.focus(); // necessary for IE >= 10
                    ALLOCATE_OBJ.mywindow.print(); // change window to winPrint
                    ALLOCATE_OBJ.mywindow.close(); // change window to winPrint
                }, 250);
            } else {
                ALLOCATE_OBJ.mywindow.document.close(); // necessary for IE >= 10
                ALLOCATE_OBJ.mywindow.focus(); // necessary for IE >= 10
                ALLOCATE_OBJ.mywindow.print();
                ALLOCATE_OBJ.mywindow.close();
            }
            return true;
        } else {
    ...
    download: function () {
        $("#exportTitle").val(ALLOCATE_OBJ.title);
        $("#windowId").val(Math.random());

        ALLOCATE_OBJ.params = new Object();
        ALLOCATE_OBJ.params.masterBeans = [];
        var masterAttrArr = [];
        var masterHeadArr = [];
        ...
        var propArr = Object.keys(ALLOCATE_OBJ.propertyMap);
        for (var p in propArr) {
            var propId = propArr[p];
            var prop = ALLOCATE_OBJ.propertyMap[propId];
            var tax = ALLOCATE_OBJ.taxDetailMap[prop.propertyUniqueId];
            if (tax === undefined) {
                prop.taxGenerated = "N";
            } else {
                prop.taxGenerated = "Y";
            ...
            for (var m in masterAttrArr) {
                ...
                if (prop[MASTERCOLUMN.attrInBean[attr]] === undefined) {
                    ...
                    if (attr === "zone") {
                        obj[attr] = ALLOCATE_OBJ.zoneMaster[prop[MASTERCOLUMN.attrInBean[attr]]];
            ...
            ALLOCATE_OBJ.params.masterBeans.push(obj);

        }

        $("#exportParams").val(JSON.stringify(ALLOCATE_OBJ.params));
        $("#exportTHead").val(JSON.stringify(masterHeadArr));
        $("#exportAttrToAdd").val(JSON.stringify(masterAttrArr));
        $("#exportType").val(ALLOCATE_OBJ.type);
        $("#expo").submit();
        $('#export-modal').modal('toggle');
    },
    searchObjResults: {},
    searchobjProperty: function (obj) {
        ALLOCATE_OBJ.searchObjResults = {};
        LOADER.show();
        $.post("loadObjectionData", {
            ...
            if (result !== undefined && result.length > 0) {
                ...
                for (var i = 0; i < result.length; i++) {
                    ...
                    prop.completeAddress = (prop.completeAddress === undefined || prop.completeAddress === null) ? "" : prop.completeAddress;
                    prop.propertyOwnerFather = (prop.propertyOwnerFather === undefined || prop.propertyOwnerFather === null) ? "" : prop.propertyOwnerFather;
                    ALLOCATE_OBJ.searchObjResults[result[i].propertyUniqueId] = result[i];
                    ...
                    tab_html += "<td>" + (prop.completeAddress) + "</td>";
                    tab_html += "<td>" + (ALLOCATE_OBJ.zoneMaster[prop.zoneId] === undefined ? "N/A" : ALLOCATE_OBJ.zoneMaster[prop.zoneId]) + "</td>";
                    tab_html += "<td style='text-align:left' onclick=\"ALLOCATE_OBJ.printObjection('"
            ...
            $.post("getWards", {zone: $("#zone").val()}, function (result) {
                if (result != undefined) {
                    ...
                    for (var i = 0; i < result.length; i++) {
                        ...
                        ALLOCATE_OBJ.wardMaster[result[i].ward] = result[i].displayName;
                    }
                    ALLOCATE_OBJ.resetWard(html);
                }
            }, 'json').always(function () {
...
$(document).on("click", "#searchProp", function () {
    ...
    if (($("#zone option:selected").val() === "-1" || $("#ward option:selected").val() === "-1")
            && $("#obj_fromDate").val() === "" && $("#obj_toDate").val() === "") {
        ...
        ALLOCATE_OBJ.searchProperty(obj);
    }

});
$(document).on("click", "#obj_submit", function () {
    var decision = $("#obj_objectionAction option:selected").val();
    var forwardTo = $("#forward_to_users option:selected").val();
    var remarks = $("#obj_decision_remarks").val();

    if (decision === "-1") {
        alert("Kindly fill decision.");
    } else if (decision === "FORWARD" && forwardTo === "-1") {
        alert("Kindly select user to forward.");
        ...
        bootbox.confirm({
            ...
            callback: function (result) {
                if (result) {
                    ALLOCATE_OBJ.commiteObjection(decision, remarks, forwardTo);
                }
            }
...
$(document).on("click", "#searchObj", function () {
    ...
    if ($("#zone option:selected").val() === "-1" && $("#obj_fromDate").val() === "" && $("#obj_toDate").val() === "") {
        ...
        ALLOCATE_OBJ.searchobjProperty(obj);
    }
...
$(document).on("change", "#zone", function () {
    ALLOCATE_OBJ.resetWard();
    ALLOCATE_OBJ.resetOccupier();
    ALLOCATE_OBJ.resetOwner();
    ALLOCATE_OBJ.resetPropertyId();
    ALLOCATE_OBJ.getWard();
});
...
$(document).on("change", "#ward", function () {
    ...
    ALLOCATE_OBJ.resetOwner();
    ALLOCATE_OBJ.resetPropertyId();
    ALLOCATE_OBJ.setSearchCriteria();
});
...
$(document).on("change", "#obj_objectionAction", function () {
    var decision = $("#obj_objectionAction option:selected").val();
    $("#forward_to_users").val("-1");
    $("#forward_to_users").closest(".SumoSelect").find(".CaptionCont span").text("");
    if (decision === "FORWARD") {
        $("#forward_to_div").removeClass("hidden");
        ALLOCATE_OBJ.getObjectionHandlerUsers();
    } else {
        $("#forward_to_div").addClass("hidden");
    }
});