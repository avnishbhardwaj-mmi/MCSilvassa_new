    ...
    loadRentableValues: function() {
        $.ajax({
            ...
            success: function(result) {
                if (result != null) {
                    var html_prop = "";
                    html_prop += "<option value='-1'>--Select Property Category--</option>";
                    for (var i = 0; i < result.length; i++) {
                        html_prop += "<option value='" + result[i].propertyRentableId + "'>" + result[i].propertyRentableId + "</option>";
                        REPORT.rentableValuesMap[result[i].propertyRentableId] = result[i];
                    }
                    $("#rep_prop_cat").html("");
                    $("#rep_prop_cat").html(html_prop);
                }
            },
            error: function(e) {
                console.log("ERROR: ", e);
...
$(document).on("click", "#searchProp", function() {
    ...
    obj.propType = $("#rep_prop_cat option:selected").val();
